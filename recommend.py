...
def find_closest(location, centroids):
    ...
    lst=list(map(distance,locs,centroids))
    min_value=min(lst)
...
def group_by_centroid(restaurants, centroids):
    ...
    for r in restaurants:
        locs.append([find_closest(restaurant_location(r),centroids),r])
    return group_by_first(locs)
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    lat=mean([restaurant_location(c)[0] for c in cluster])
    lon=mean([restaurant_location(c)[1] for c in cluster])
    return [lat,lon]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    mean_x=mean(xs)
    mean_y=mean(ys)
    sxx=sum(list(map(lambda x: x**2,list(map(lambda x: x-mean_x, xs)))))
    syy=sum(list(map(lambda x: x**2,list(map(lambda x: x-mean_y, ys)))))
    sxy=sum(list(map(lambda x,y: x*y , list(map(lambda x: x-mean_x, xs)),list(map(lambda x: x-mean_y, ys)))))
    b=sxy/sxx
    a=mean_y-b*mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        pres.append(list(find_predictor(user,reviewed,f)))
    return max(pres,key=lambda x:x[1])[0]
    # END Question 8
